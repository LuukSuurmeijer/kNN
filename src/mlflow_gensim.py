"""
An MLflow GenSim model flavor does not exist yet. This file is largely copied from the
code in [this](https://github.com/mlflow/mlflow/issues/2770) issue on the MLflow repo,
which has been stale since 2020.
As quick fix is implemented in `_load_model_from_local_file`: the code in the issue uses
pickle to load a general GenSim model, but this fails for Doc2Vec models. The fix
assumes the model is a Doc2Vec model.
